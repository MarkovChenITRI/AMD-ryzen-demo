...
AMD Ryzen APU雖然已經搭載了完整的internal GPU(=iGPU)及NPU，但是這些處理器僅支援與"推論"有關的流程，針對"訓練"方面的工作仍只能依靠內建的CPU。或者，您也可以透過外部擴充槽圖形卡(如：[Radeon](https://www.amd.com/en/products/graphics/desktops/radeon.html))，以此增強本機在訓練模型、微調神經網路參數的能力。
